...
- [ ] **Step 4: Create the Profile model and the User relation**
...
- [ ] **Step 5: Create the UpdateProfileRequest**
...
- [ ] **Step 6: Create the ProfileController and register routes**
...
- [ ] **Step 1: Fix the pre-existing mock gap and write the failing tests**
...
- [ ] Run the full backend suite: `cd backend && composer test` — expect all green, including the 9 new Profile tests.
- [ ] Run the full frontend suite: `cd frontend && npx vitest run` — expect all green, including the previously-crashing pre-existing tests now fixed by the `getZoom` mock addition.
- [ ] Manually verify in the browser (`composer dev` from `backend/`, or `npm run dev` from `frontend/` against a running API): grant geolocation, click the avatar, confirm "Yes", see the orange home marker and Home button appear, pan away, click Home to fly back. Then deny/disable geolocation and reload — confirm the map centers on the home base instead of the contiguous-US default.
